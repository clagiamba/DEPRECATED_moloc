...
match_alleles <- function(data, A1.ref="A1.ref", A2.ref="A2.ref",  flip = TRUE) {
   ...
   match_flip = data[,A1.ref] == data[,"A2"] & data[,A2.ref] == data[,"A1"]
   match_comp_one = data[,A1.ref] == complement_snp(data[,"A1"]) & data[,A2.ref]== complement_snp(data[,"A2"])
   match_comp_two = data[,A1.ref] == complement_snp(data[,"A2"]) & data[,A2.ref] == complement_snp(data[,"A2"])
   ...
   if (flip) {
           if (any(which(match_flip)>0)) {
               data[match_flip, "A1"]=data[match_flip, A1.ref]
               data[match_flip, "A2"]=data[match_flip, A2.ref]
               data[match_flip, "BETA"]=-data[match_flip, "BETA"]
           }
   }
